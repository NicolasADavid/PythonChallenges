...
class Solution:
    def pacificAtlantic(self, grid: List[List[int]]) -> Set[Tuple[int,int]]:
        ...
        def goAllWays(h, x, y, targetSet) -> None:
            # up
            goUp(h, x, y-1, targetSet)

            # down
            goUp(h, x, y+1, targetSet)

            # right
            goUp(h, x+1, y, targetSet)

            # left
            goUp(h, x-1, y, targetSet)
                
        def goUp(h, x, y, targetSet: Set) -> None:
            ...
            goAllWays(cellHeight, x, y, targetSet)
        ...
        # right
        for i in rowRange:
            goUp(0, i, cols-1, seenAtlantic)

        # bottom
        for i in colRange:
            goUp(0, rows-1, i, seenAtlantic)

        #### goPacific

        # left
        for i in rowRange:
            goUp(0, i, 0, seenPacific)

        # top
        for i in colRange:
            goUp(0, 0, i, seenPacific)

        return seenAtlantic & seenPacific
